...
const ChuyenhuongKTab = ({navigation}) => {
  ...
  const filteredViolations = fullViolations.filter(item =>
    item.topicCode === 2 && item.entities.includes('Chủ phương tiện')||
    item.entities.includes('xe xích lô')||
    item.entities.includes('xe đạp máy')||
    item.entities.includes('xe đạp')||
    item.entities.includes('Người từ đủ 14 tuổi đến dưới 16 tuổi')||
    item.entities.includes('máy kéo')
  );
