...
const HieulenhchidanKTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 1 && item.violation.includes("chấp hành")
  );
...
const HieulenhchidanKStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="HieulenhchidanKTab"
        component={HieulenhchidanKTab}
        options={({ navigation }) => ({
...
export default HieulenhchidanKStack;
