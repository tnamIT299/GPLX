...
const NongdoconKTab = ({navigation}) => {
  const [searchText, setSearchText] = useState('');

  const filteredViolations = fullViolations.filter(item =>
    item.topicCode === 9 && 
    (item.violation.includes('nồng độ cồn') || item.violation.includes('ma tuý'))
  );
...
const NongdoconKStack = () => {
  return (
    <Stack.Navigator>
    <Stack.Screen
      name="NongdoconKTab"
      component={NongdoconKTab}
      options={({ navigation }) => ({
        title: 'Nồng độ cồn, chất kích thích',
...
export default NongdoconKStack;
