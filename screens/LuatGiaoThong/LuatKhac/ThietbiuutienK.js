...
const ThietbiuutienKTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 4 && item.violation.includes("còi")
  );
...
const ThietbiuutienKStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="ThietbiuutienKTab"
        component={ThietbiuutienKTab}
        options={({ navigation }) => ({
...
export default ThietbiuutienKStack;
