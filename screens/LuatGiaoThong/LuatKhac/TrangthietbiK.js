...
const TrangthietbiKTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 7 && item.violation.includes("đèn")
  );
...
const TrangthietbiKStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="TrangthietbiKTab"
        component={TrangthietbiKTab}
        options={({ navigation }) => ({
...
export default TrangthietbiKStack;
