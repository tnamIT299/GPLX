...
const VanchuyenKTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 6 && item.violation.includes("Chở")
  );
...
const VanchuyenKStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="VanchuyenKTab"
        component={VanchuyenKTab}
        options={({ navigation }) => ({
...
export default VanchuyenKStack;
