...
const GiaytoxeOTTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 10 && item.entities.includes("xe ô tô")
  );
...
const GiaytoxeOTStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="GiaytoxeOTTab"
        component={GiaytoxeOTTab}
        options={({ navigation }) => ({
          title: "Giấy tờ xe",
...
export default GiaytoxeOTStack;
