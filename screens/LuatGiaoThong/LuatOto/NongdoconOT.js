...
const NongdoconOTTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 9 && item.entities.includes("xe ô tô")
  );
...
const NongdoconOTStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="NongdoconOTTab"
        component={NongdoconOTTab}
        options={({ navigation }) => ({
...
export default NongdoconOTStack;
