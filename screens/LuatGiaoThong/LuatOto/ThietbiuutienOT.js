...
const ThietbiuutienOTTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 4 && item.entities.includes("xe ô tô")
  );
...
const ThietbiuutienOTStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="ThietbiuutienOTTab"
        component={ThietbiuutienOTTab}
        options={({ navigation }) => ({
...
export default ThietbiuutienOTStack;
