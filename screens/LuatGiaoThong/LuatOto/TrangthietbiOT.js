...
const TrangthietbiOTTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 7 && item.entities.includes("xe ô tô")
  );
...
const TrangthietbiOTStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="TrangthietbiOTTab"
        component={TrangthietbiOTTab}
        options={({ navigation }) => ({
...
export default TrangthietbiOTStack;
