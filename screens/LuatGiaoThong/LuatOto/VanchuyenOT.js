...
const VanchuyenOTTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 6 && item.entities.includes("xe ô tô")
  );
...
const VanchuyenOTStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="VanchuyenOTTab"
        component={VanchuyenOTTab}
        options={({ navigation }) => ({
...
export default VanchuyenOTStack;
