...
const ChuyenhuongXMTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 2 && item.entities.includes("xe mô tô")
  );
...
const ChuyenhuongXMStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="ChuyenhuongXMTab"
        component={ChuyenhuongXMTab}
        options={({ navigation }) => ({
...
export default ChuyenhuongXMStack;
