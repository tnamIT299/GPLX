...
const DuongcamXMTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 8 && item.entities.includes("xe mô tô")
  );
...
const DuongcamXMStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="DuongcamXMTab"
        component={DuongcamXMTab}
        options={({ navigation }) => ({
...
export default DuongcamXMStack;
