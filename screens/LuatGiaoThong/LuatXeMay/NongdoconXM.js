...
const NongdoconXMTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 9 && item.entities.includes("xe mô tô")
  );
...
const NongdoconXMStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="NongdoconXMTab"
        component={NongdoconXMTab}
        options={({ navigation }) => ({
...
export default NongdoconXMStack;
