...
const TocdoXMTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 5 && item.entities.includes("xe mô tô")
  );
...
const TocdoXMStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="TocdoXMTab"
        component={TocdoXMTab}
        options={({ navigation }) => ({
          title: "Tốc độ, khoảng cách",
