...
const TrangthietbiXMTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 7 && item.entities.includes("xe mô tô")
  );
...
const TrangthietbiXMStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="TrangthietbiXMTab"
        component={TrangthietbiXMTab}
        options={({ navigation }) => ({
          title: "Trang thiết bị phương tiện",
...
export default TrangthietbiXMStack;
