...
const VanchuyenXMTab = ({ navigation }) => {
  const [searchText, setSearchText] = useState("");

  const filteredViolations = fullViolations.filter(
    (item) => item.topicCode === 6 && item.entities.includes("xe mô tô")
  );
...
const VanchuyenXMStack = () => {
  return (
    <Stack.Navigator>
      <Stack.Screen
        name="VanchuyenXMTab"
        component={VanchuyenXMTab}
        options={({ navigation }) => ({
...
export default VanchuyenXMStack;
